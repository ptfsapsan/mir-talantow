$(function (){
   ...
   // // переключение номинаций и тем в мобильной версии
   // $('select[name=nomination], select[name=theme]').on('change', function(){
   //    location.href = $(this).val();
   // });

   // переключение номинаций и тем
   $('[name=nomination_id], [name=theme_id]').on('change', function(){
      var type = $('[name=type]').val(),
         link = '/ajax/redirect?type=' + type + '&nomination_id=' +
            $('[name=nomination_id]').val();
      if(type == 'kid') link += '&theme_id=' + $('[name=theme_id]').val();
